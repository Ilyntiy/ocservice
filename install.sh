...
if [[ -L "/usr/local/bin/ocservice" ]]; then
  SYMLINK_TARGET=$(readlink -f "/usr/local/bin/ocservice")
  CANDIDATE="$(dirname "$SYMLINK_TARGET")/ocservice.conf"
  if [[ -f "$CANDIDATE" ]]; then
    INSTALL_DIR="$(dirname "$SYMLINK_TARGET")"
    CONF_FILE="$CANDIDATE"
  fi
fi
...
NAMESEOF
  ...
  info "Skipped (already exists): $INSTALL_DIR/names"
fi
...
if [[ -f "$CONF_FILE" ]]; then
  info "Existing config found — patching"
  if ! grep -q "^CERT_CACHE_FILE" "$CONF_FILE"; then
    {
      echo ""
      echo "# ============================================================================="
      ...
      echo "# ============================================================================="
      echo ""
      echo "# Stores certificate dates for fast User Management Center loading."
      echo "# Managed automatically by gen-client and user-center."
      echo "# If you create certificates outside of ocservice, run"
      echo "# 'r — Rebuild cache' in User Management Center to update it."
      echo "CERT_CACHE_FILE=$INSTALL_DIR/cert_cache"
    } >> "$CONF_FILE"
    info "Added: CERT_CACHE_FILE"
  else
...
# Stores certificate dates for fast User Management Center loading.
# Managed automatically by gen-client and user-center.
# If you create certificates outside of ocservice, run
# 'r — Rebuild cache' in User Management Center to update it.
...
# cert_cache
if [[ ! -f "$INSTALL_DIR/cert_cache" ]]; then
  touch "$INSTALL_DIR/cert_cache"
fi
chown "$REAL_USER:$REAL_USER" "$INSTALL_DIR/cert_cache"
chmod 644 "$INSTALL_DIR/cert_cache"
info "cert_cache: $INSTALL_DIR/cert_cache"
...
if [[ "$AUTH_MODE" != "plain" ]]; then
  echo
  warn "If you already have certificate users, open User Management Center"
  warn "and run 'r — Rebuild cache' to initialize the certificate cache."
fi
echo
divider
echo "  To uninstall:"
